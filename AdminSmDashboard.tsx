...
import { apiGet, apiGetNoAuth, apiPost, apiPatch, apiStreamPdf } from '../utils/api';
...
const AdminSmDashboard: React.FC = () => {
  ...
  const [apiCallStates, setApiCallStates] = useState<{
    cmCodes: boolean;
    masterData: boolean;
  }>({
    cmCodes: false,
  ...
  // Manage overall loading state based on individual API call states
  useEffect(() => {
    const allApiCallsComplete = apiCallStates.cmCodes && apiCallStates.masterData;
    setLoading(!allApiCallsComplete);
  }, [apiCallStates]);
  ...
  useEffect(() => {
    const fetchCmCodes = async () => {
      try {
        setApiCallStates(prev => ({ ...prev, cmCodes: false }));
        setError(null);
        ...
        if (result.success) {
          ...
          // Add is_active field if missing from API response and map region_id to region_name
          console.log('Processing CM codes with regions data:', regions);
          const processedData = result.data.map(item => {
            // Find region name from region_id
            let regionName = null;
            console.log(`Processing item ${item.cm_code} with region_id: ${item.region_id}, regions available: ${regions.length}`);
            if (item.region_id && regions.length > 0) {
              const region = regions.find(r => r.id === item.region_id);
              regionName = region ? region.name : null;
              console.log(`Found region for ${item.cm_code}:`, region, 'mapped name:', regionName);
            } else {
              console.log(`No region mapping for ${item.cm_code} - region_id: ${item.region_id}, regions length: ${regions.length}`);
            }
            
            return {
              ...item,
              is_active: item.is_active !== undefined ? item.is_active : true, // Default to true if missing
              region_name: regionName || item.region_name // Use mapped region_name or existing region_name
            };
          });
          ...
          // Extract unique signoff statuses for filter
        ...
        setApiCallStates(prev => ({ ...prev, cmCodes: true }));
      }
    };
  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      try {
        setApiCallStates(prev => ({ ...prev, masterData: false }));
        ...
        if (result.success && result.data) {
           ...
           if (result.data.periods && Array.isArray(result.data.periods)) {
             ...
             // Sort periods by ID in descending order (3, 2, 1)
             const sortedPeriods = [...processedPeriods].sort((a, b) => {
               return b.id - a.id; // Descending order (3, 2, 1)
             });
             
             setPeriods(sortedPeriods);
             console.log('Available periods (sorted desc):', sortedPeriods);
             
             // Set current period as default (most recent period)
             if (sortedPeriods.length > 0) {
               const currentPeriod = sortedPeriods[0]; // First item is already the highest ID
          ...
          // Process regions
          console.log('Raw regions data from API:', result.data.regions);
          if (result.data.regions && Array.isArray(result.data.regions)) {
            setRegions(result.data.regions);
            console.log('Available regions:', result.data.regions);
          } else {
            console.log('No regions data found in API response or not an array');
          }
        ...
        setApiCallStates(prev => ({ ...prev, masterData: true }));
      }
    };
  ...
  // Assuming `periods` is already available and populated
const highestPeriodId = periods.length > 0 ? Math.max(...periods.map(p => p.id)) : "";

useEffect(() => {
  setSelectedPeriod(highestPeriodId.toString());
}, [periods]);
  ...
  const handleExportToExcel = () => {
    const exportData = currentData.map(row => ({
      ...
      'Signoff Status': row.signoff_status === 'signed'
        ...
        ? 'Pending'
        : '',
      'Spocs': '', // Currently empty as spocs data is not available in CmCode interface
      'Is Active': row.is_active ? 'Yes' : 'No'
    }));

    // Generate filename with timestamp
    const timestamp = new Date().toISOString().split('T')[0];
    const filename = `admin-sm-dashboard-export-${timestamp}`;
    
    // Export to Excel using secure utility (falls back to CSV if needed)
    exportToExcel(exportData, filename, 'Admin SM Dashboard Data');
  };
  ...
  const fetchSignoffDetails = async (cmCode: string, period?: string) => {
    try {
      ...
      let url = `/signoff-details/cm-period?cm_code=${encodeURIComponent(cmCode)}`;
      if (period) {
  ...
  const handleOpenAdd3PMModal = () => {
    ...
    // Find the latest period (highest ID value)
    let latestPeriodId = '';
    if (periods.length > 0) {
      const sortedPeriods = [...periods].sort((a, b) => b.id - a.id);
      latestPeriodId = sortedPeriods[0].id.toString();
    }
    
    setNew3PMData({
      period: latestPeriodId,
      srm_name: '',
      srm_email: '',
      cm_code: '',
      cm_description: '',
  ...
  const handleAdd3PMSave = async () => {
    ...
    // Call API directly
    await handleAdd3PMSaveDirect();
  };

  // Handle Add 3PM form submission directly
  const handleAdd3PMSaveDirect = async () => {
    try {
      ...
      fetchData();
          ...
          if (err instanceof Error) {
            ...
            if (errorMessage.includes('400')) {
              ...
              errorMessage = 'This 3PM Code already exists in the system. Please use a different 3PM Code.';
              // Clear the 3PM Code field to help user enter a new one
              setNew3PMData(prev => ({ ...prev, cm_code: '' }));
            } else if (errorMessage.includes('duplicate key value violates unique constraint') || errorMessage.includes('username_key')) {
              errorMessage = 'A user with this email already exists in the system. Please use different email addresses for SRM and spokes, or contact support if you believe this is an error.';
              // Clear the email fields to help user enter new ones
              setNew3PMData(prev => ({ 
                ...prev, 
                srm_email: '',
                spokes: prev.spokes.map(spoke => ({ ...spoke, email: '' }))
              }));
          ...
          console.log('Error set in state:', errorMessage);
    } finally {
      setAdd3PMLoading(false);
      console.log('Loading set to false, modal should stay open');
    }
  };
  ...
  const handleIsActiveChange = async (cmCode: string, currentStatus: boolean, id: number) => {
    try {
      ...
      const apiUrl = `/cm-codes/${id}/status`;
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: '20px' }}>
          ...
          <div style={{ marginTop: '10px', fontSize: '14px', color: '#666' }}>
            {!apiCallStates.cmCodes && 'Loading CM codes...'}
            {!apiCallStates.masterData && 'Loading master data...'}
          </div>
        </div>
      )}
      {toggleError && (
                ...
                <div style={{ flex: 1, minWidth: 160 }}>
                  ...
                  <MultiSelect
                    options={regions.map(region => ({
                      value: region.name,
                      label: region.name.charAt(0).toUpperCase() + region.name.slice(1)
                    }))}
                        ...
                        <div style={{ fontSize: '12px', fontWeight: '500', color: '#333' }}>
                          {record.signed_pdf_url ? (
                            <button
                              onClick={async () => {
                                try {
                                  console.log('Streaming PDF:', record.signed_pdf_url);
                                  
                                  // Get access token from localStorage
                                  const accessToken = localStorage.getItem('access_token') || undefined;
                                  
                                  // Stream the PDF through backend
                                  const response = await apiStreamPdf(record.signed_pdf_url, accessToken);
          ...
          <div className="add-3pm-modal" style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <label style={{
                      ...
                      <span 
                        ...
                        onMouseEnter={(e) => {
                          showTooltip("This field is auto-selected to the latest period and cannot be changed", e);
                          e.currentTarget.style.color = '#30ea03';
                        }}
                        onMouseLeave={(e) => {
                          hideTooltip();
            ...
            {add3PMError && (
              <div style={{
                ...
                <i className="ri-error-warning-line" style={{ fontSize: '16px' }}></i>
                {add3PMError}
              </div>
            )}
