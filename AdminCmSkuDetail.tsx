...
import { apiGet, apiGetNoAuth, apiPost, apiPut, apiPatch, apiPostFormData, apiPutFormData } from '../utils/api';
...
const AdminCmSkuDetail: React.FC = () => {
  ...
  const fetchDashboardData = async (
    // includeParams: string[] = ['skus', 'descriptions', 'references', 'audit', 'masterdata'],
    includeParams: string[] = ['skus', 'descriptions', 'references', 'audit_logs', 'component_data', 'master_data'],
   ...
   * Replaces: GET /masterdata
   */
  const fetchMasterData = async () => {
    if (!cmCode) return;
    
    try {
      const data = await fetchDashboardData(['masterdata']);
  ...
  useEffect(() => {
    const fetchDirectMasterData = async () => {
      try {
        console.log('Fetching master data directly from /masterdata API');
  ...
  const handleViewComponentHistory = async (component: any) => {
    ...
    try {
      ...
      const result = await apiGet(`/component-audit-log/${componentId}`);
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const exportData = componentData.map((component: any) => ({
        ...
        'CM Description': component.sku_code || '',
        ...
        'Component Validity From': component.component_valid_from || '',
        'Component Validity To': component.component_valid_to || '',
        ...
        'Component Base Unit of Measure': component.component_base_uom_id || '',
        'Component %w/w': component.percent_w_w || '',
        'Evidence': component.evidence || '',
        'Component Packaging Type': component.component_packaging_type_id || '',
        'Component Packaging Material': component.helper_column || '',
        ...
        'Last Update Date': component.last_update_date || ''
